...
/**
 * Tests that verify a set of text output produced by legacy code.
 * Verify that the output it the same for refactored code.
 * This is a failsafe for refactoring.
 * During refactoring we can intruduce unit tests that will document behaviour.
 * When refactoring is Ok (meaning that no existing behaviour is changed) and there is no new unit test to add,
 * we can implement the missing feature (spoiler : it will break those tests because legacy don't implement them.
 */
public class ApprovalTest {

    @Test
    void legacyBehaviour2Days() {
        assertThat(executeLegacy(2, initialItems()))
                .isEqualToNormalizingNewlines(characterizedLegacyOutputFor2Days());
    }

    @Test
    void refactoredBehaviour2Days() {
        assertThat(executeRefactored(2, initialItems()))
                .isEqualToNormalizingNewlines(characterizedLegacyOutputFor2Days());
    }

    @Test
    void legacyBehaviour31Days() {
        assertThat(executeLegacy(31, initialItems()))
                .isEqualToNormalizingNewlines(characterizedLegacyOutputFor31Days());
    }

    @Test
    void refactoredBehaviour31Days() {
        assertThat(executeRefactored(31, initialItems()))
                .isEqualToNormalizingNewlines(characterizedLegacyOutputFor31Days());
    }
    ...
    private String executeLegacy(int days, Item[] items) {
    ...
    private String executeRefactored(int days, Item[] items) {
        ByteArrayOutputStream out1 = new ByteArrayOutputStream();
        PrintStream out = new PrintStream(out1);
        out.println("OMGHAI!");
    ...
    private String characterizedLegacyOutputFor2Days() {
        return "OMGHAI!\n" +
                "-------- day 0 --------\n" +
                "name, sellIn, quality\n" +
    ...
    private String characterizedLegacyOutputFor31Days() {
        return "OMGHAI!\n" +
                "-------- day 0 --------\n" +
                "name, sellIn, quality\n" +
                "+5 Dexterity Vest, 10, 20\n" +
